...
def markdown_to_html(value):
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if in_code:
                code = esc("\n".join(code_lines))
                label = f'<div class="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">{esc(code_lang)}</div>' if code_lang else ""
                out.append(f'<div class="code-wrap">{label}<pre><code>{code}</code></pre></div>')
                in_code = False
                code_lines = []
                code_lang = ""
            else:
    ...
    if in_code:
        code = esc("\n".join(code_lines))
        out.append(f'<div class="code-wrap"><pre><code>{code}</code></pre></div>')
    flush_paragraph()
    close_list()
    return "\n".join(out)
...
def render_code(block):
    ...
    label = f'<div class="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">{esc(language)}</div>' if language else ""
    return f'<div class="code-wrap">{label}<pre><code>{esc(block.get("content", ""))}</code></pre></div>'


def render_block(block, charts):
...
def render_doc(doc):
    ...
    for section_index, section in enumerate(sections):
        ...
        for block in blocks:
            parts.append(render_block(block, charts))
...
<head>
  ...
  <style>
    ...
    .code-wrap {{ border: 1px solid #e2e8f0; border-radius: 1rem; background: #fff; padding: 1rem; box-shadow: 0 1px 2px rgb(15 23 42 / 0.04); }}
    .code-wrap pre {{ overflow-x: auto; border-radius: 0.75rem; background: #0f172a; color: #e2e8f0; padding: 1rem; font-size: 0.875rem; line-height: 1.6; }}
  </style>
</head>
